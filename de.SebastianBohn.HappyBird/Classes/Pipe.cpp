...
void Pipe::spawnPipe(cocos2d::Layer *layer){
    ...
    topPipe->cocos2d::Node::setRotation(180.0f);
    ...
    auto topPipeAction = MoveBy::create(PIPE_MOVEMENT_SPEED * visibleSize.width, Point( -visibleSize.width * 1.5f, 0));
    auto bottomPipeAction = MoveBy::create(PIPE_MOVEMENT_SPEED * visibleSize.width, Point( -visibleSize.width * 1.5f, 0));
    ...
    scoreNode->setPosition(Point(topPipe->getPositionX() + topPipe->getContentSize().width / 2, topPipe->getPositionY() -
                                 (topPipe->getContentSize().height / 2) - ((Sprite::create("sprite_1.png")->getContentSize().height + 7 * PIPE_GAP) / 2)));
    
    layer->addChild(scoreNode);
    auto pointNodeAction = MoveBy::create(PIPE_MOVEMENT_SPEED * visibleSize.width, Point(-visibleSize.width * 1.5f, 0));
